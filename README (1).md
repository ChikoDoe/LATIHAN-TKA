...
![Anthropic](https://img.shields.io/badge/AI-Claude%20Haiku-D97706?style=flat)
![OpenAI](https://img.shields.io/badge/AI-GPT--4o%20mini-74AA9C?style=flat&logo=openai&logoColor=white)
...
- **30 soal per sesi** — di-generate AI saat sesi dimulai, soal pertama langsung tampil
...
- **Pilihan provider AI** — Anthropic (Claude Haiku) atau OpenAI (GPT-4o mini)
...
### Otomatis (Linux/macOS) — Recommended
...
3. **Pilih provider AI** (Anthropic atau OpenAI)
4. Input API key
5. Pilih mode jalankan (foreground / PM2 / manual)
...
# 3. Buat file .env
cp .env.example .env
nano .env  # isi provider dan API key
...
Buat file `.env` berdasarkan `.env.example`. Pilih salah satu provider:

### Menggunakan Anthropic (Claude Haiku)
...
|----------|------------|---------|
| `API_PROVIDER` | Provider AI: `anthropic` atau `openai` | `anthropic` |
| `ANTHROPIC_API_KEY` | API key dari [console.anthropic.com](https://console.anthropic.com/settings/keys) | *(wajib jika pakai Anthropic)* |
| `OPENAI_API_KEY` | API key dari [platform.openai.com](https://platform.openai.com/api-keys) | *(wajib jika pakai OpenAI)* |
| `OPENAI_MODEL` | Model OpenAI yang dipakai | `gpt-4o-mini` |
...
|---|---|---|
| Harga estimasi | ~$0.001/soal | ~$0.001/soal |
| Daftar | [console.anthropic.com](https://console.anthropic.com) | [platform.openai.com](https://platform.openai.com) |
| Top up | [Plans & Billing](https://console.anthropic.com/settings/billing) | [Billing](https://platform.openai.com/settings/billing) |

> **Ganti provider kapan saja** cukup dengan mengubah `API_PROVIDER` di `.env` lalu restart server — tidak perlu install ulang.
...
```
LATIHAN-TKA/
├── server.js          # Backend Express + SQLite + multi-provider AI
├── package.json
...
Server menampilkan log detail di terminal untuk memudahkan debugging:
...
[CONFIG] Port     : 3000
==================================================
[START] TKA Latihan v2 → http://localhost:3000
[GEN] soal=1 elemen=Bilangan level=2 attempt=1
[OPENAI] model=gpt-4o-mini finish_reason=stop prompt=312 completion=198
[OK] soal=1 selesai 1243ms
[429] rate limited soal=3, tunggu 3000ms
[FATAL] soal=5 status=401: {"error":{"message":"Incorrect API key"}}
```
...
pm2 logs latihan-tka          # lihat log real-time
...
Secara default server hanya bisa diakses dari `localhost`. Untuk akses dari device lain di jaringan yang sama, ubah di `server.js` baris terakhir:
...
| AI (pilihan 1) | Anthropic Claude Haiku |
| AI (pilihan 2) | OpenAI GPT-4o mini |
